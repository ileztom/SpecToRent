...
interface MachineryDetailsPageProps {
  machinery: Machinery;
  onBack: () => void;
  onCreateOrder: (data: { startDate: string, endDate: string, quantity: number, address: string }) => void;
  user: User | null;
  onLogin: () => void;
}
...
export const MachineryDetailsPage: React.FC<MachineryDetailsPageProps> = ({
  ...
  const isOwnMachinery = user && machinery.ownerId === Number(user.id);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!address.trim()) {
      alert("Укажите адрес доставки техники");
      return;
    }
    onCreateOrder({ startDate, endDate, quantity, address });
  };
                   ...
                   {isOwnMachinery ? (
                      <>
                        <p className="text-sm text-gray-500">
                          Это ваше объявление. Вы не можете арендовать собственную технику.
                        </p>
                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                          <p className="text-sm text-yellow-700">
                            Управляйте этим объявлением в разделе "Моя техника"
                          </p>
                        </div>
                      </>
                   ) : (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                   ...
                   <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">
                        Адрес доставки техники
                      </label>
                      <input 
                        type="text"
                        required
                        placeholder="Введите адрес, где нужна техника"
                        className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-900"
                        value={address}
